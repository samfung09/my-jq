(function(window){
    ...
    function JQ(arg){
        ...
        }else if(arg instanceof HTMLElement || arg instanceof HTMLDocument || arg instanceof Window){
            this.els = [arg];
        }
    }
    ...
    JQ.prototype.show = function(){
        for(var i=0; i<this.els.length; i++){
            var display = '';
            if(this.els[i].display == 'none' || typeof this.els[i].display == "undefined"){
                display = defaultDisplay(this.els[i]);
            }else{
                display = this.els[i].display;
            }
    ...
    //滚动事件
    JQ.prototype.scroll = function(fn){
        for(var i=0; i<this.els.length; i++){
            this.els[i].onscroll = fn;
        }
    }
    //动态绑定事件
